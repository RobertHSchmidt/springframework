...
/**
 * Abstract base class for objects handling {@link FlowExecutor} arguments.
 * This class combines the two argument handling responsabilities
 * ({@link FlowExecutorArgumentExtractor argument extraction}
 * and {@link FlowExecutorArgumentExposer exposing arguments}) and makes sure
 * they are consistent, i.e. that exposed arguments can later be extracted
 * again.
 * <p>
 * All argument names are configurable. Common convenience functionality is
 * also provided, e.g. a {@link #applyDefaultFlowId(String) default flow id},
...
public abstract class FlowExecutorArgumentHandler implements FlowExecutorArgumentExtractor, FlowExecutorArgumentExposer {
	 ...
	 */
	private static final String FLOW_ID_ARGUMENT = "_flowId";

	/**
	 * By default clients can send the key of a flow execution to be resumed
	 * using an argument with this name ("_flowExecutionKey").
	 */
	private static final String FLOW_EXECUTION_KEY_ARGUMENT = "_flowExecutionKey";

	/**
	 * By default clients can send the event to be signaled in an argument with
	 * this name ("_eventId").
	 */
	private static final String EVENT_ID_ARGUMENT = "_eventId";

	/**
	 * Identifies a flow definition to launch a new execution for, defaults to
	 * {@link #FLOW_ID_ARGUMENT}.
	 */
	private String flowIdArgumentName = FLOW_ID_ARGUMENT;

	/**
	 * Input argument that identifies an existing flow execution to participate
	 * in, defaults to {@link #FLOW_EXECUTION_KEY_ARGUMENT}.
	 */
	private String flowExecutionKeyArgumentName = FLOW_EXECUTION_KEY_ARGUMENT;

	/**
	 * Identifies an event that occured in an existing flow execution, defaults
	 * to {@link #EVENT_ID_ARGUMENT}.
	 */
	private String eventIdArgumentName = EVENT_ID_ARGUMENT;
	...
	/**
	 * Sets the event id argument name, used to signal what user action
	 * happened within a paused flow execution.
	 */
	...
	/**
	 * Returns the <i>default</i> flowId argument value. If no flow id
	 * argument is provided, the default acts as a fallback. Defaults to
	 * <code>null</code>.
	 */
	 ...
	 * <p>
	 * This value will be used if no flowId argument value can be extracted
	 ...
	 * @param extractedFlowId the extracted flow id, could be null if non
	 * was available in the external context
	 ...
	 * with a slash ("/") it will be made relative to the current ServletContext,
	 * i.e. relative to the web application root.
