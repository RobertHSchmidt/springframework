...
public class JavaConfigWebApplicationContextTests {
	...
	// unfortunately, we can't expect to throw an exception on a bogus class, because the parsing
	// algorithm assumes that something that is not a class must be a base package pattern
	// TODO: revisit this - can we make a clear distinction between classes and packages? would be
	// nice to be able to expect ClassNotFoundException / IllegalArgumentException when a user typos
	// a class. suggestion: check location for '/' or '*' - this guarantees that it's intended to be
	// a base package. If there are no wildcards provided, then try to instantiate a package via
	// Package.getPackage(location); if that returns null, then try Class.forName(location). If that
	// throws ClassNotFoundException, then throw new IllegalArgumentException("location could not be
	...
	/**
	 * Tests that a JavaConfigWebApplicationContext must have refresh() called on it before being
	 * used. refresh() is called as part of the lifecycle of {@link DispatcherServlet} and
	 * {@link ContextLoaderListener}. Typically users would not manually instantiate this class.
	 */
